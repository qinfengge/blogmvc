...
<form action="article?opr=updart&aid=${artic.articleId }" enctype="multipart/form-data" method="post">
<h1 id="opt_type"> 修改新闻： </h1>
	文章名：<input type="text" name="artname" value="${artic.articleName}"><br>
	文章标签：<select name="atag">
        	<option value="0">请选择</option>
        	<c:forEach items="${listtag }" var="tag">
        		<option value='${tag.tagId }'> ${tag.tagName } </option>
        		<c:if test="${tag.tagId==artic.tagId }">
        			<option value='${tag.tagId }' selected="selected"> ${tag.tagName }</option>
        		</c:if>
        	</c:forEach>
        	</select><br>
    作者：<input type="text" name="author" value="${artic.author}"><br>
	文章内容：<textarea name="acontent" cols="70" rows="10">${artic.articleContent}</textarea><span class="errorMsg"></span><br>
	文章图片：<input name="file" type="file" class="opt_input" />(只能上传"gif", "jpg", "jpeg"格式图片)<br>
	<img src="${artic.imgSrc }" width="20%"></img><br>
